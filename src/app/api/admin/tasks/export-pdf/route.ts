...
import { ALL_EVENTS_VALUE, getActiveEventId } from "@/lib/event-context";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const printUrl = `${baseUrl}/tasks/print?eventId=${eventId}`;
    ...
    const safeEvent = (eventName || "Event").toLowerCase().replace(/[^a-z0-9]+/g, "_");
    const filename = `Chathurya_Oshadi_${safeEvent}_Tasks.pdf`;
