...
export async function GET(
  ...
  try {
    ...
    const printUrl = `${baseUrl}/tasks/${taskId}/print`;
    ...
    const safeEvent = (task.event?.name || "Event").toLowerCase().replace(/[^a-z0-9]+/g, "_");
    const safeTitle = (task.title || "Task").toLowerCase().replace(/[^a-z0-9]+/g, "_").substring(0, 30);
    const filename = `Chathurya_Oshadi_${safeEvent}_Task_${safeTitle}.pdf`;
